...
const _checksum = (address: string) => {
  ...
  const addressHash = CryptoHelper.keccak256(address.toLowerCase())
  for (let i = 0; i < 40; i++) {
    // The nth letter should be uppercase if the nth digit of casemap is 1
    const upperCase = parseInt(addressHash[i], 16) > 7 && address[i].toUpperCase() !== address[i]
    const lowerCase = parseInt(addressHash[i], 16) <= 7 && address[i].toLowerCase() !== address[i]
